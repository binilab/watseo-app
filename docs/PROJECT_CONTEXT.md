...
현재는 QR 도착 확인에 카메라 스캔과 수동 입력 fallback을 함께 제공하며, 위치 권한과 푸시 알림은 아직 붙이지 않은 상태다.
...
- `/home/qr-arrival` 카메라 QR 스캔 및 수동 입력 fallback
...
- 주요 실패 상황에서 raw error 대신 사용자용 안내 문구 표시
- 공통 `showFriendlyAlert` 기반 확인/실패 안내
